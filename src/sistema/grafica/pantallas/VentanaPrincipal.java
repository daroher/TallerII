...
//		frame.getContentPane().add(lblNewLabel, BorderLayout.CENTER);
		...
		/**
		 * creo para cada item de los menus un actionListener, el metodo privado
		 * crearActionListener se encarga de definir que formulario se va a mostrar
		 * segun que item se de click. Es el que se le pasa por parametro El primer
		 * numero es el indice del menu padre (Asignaturas,Alumnos,Inscripciones), el
		 * segundo indice es de cada sub-menu hijo de esos 3
		 **/
	...
	private void vaciarCampos(JPanel form) {
		for (Component component : form.getComponents()) {
			if (component instanceof JTextField) {
				((JTextField) component).setText("");
			}else if (component instanceof JPanel) {
				vaciarCampos((JPanel) component);
			}
		}
	}
}
